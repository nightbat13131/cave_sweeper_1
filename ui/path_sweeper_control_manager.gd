...
func _on_selected(control: PathSweeperPressTypeControlSelector, mouse_mask: int) -> void:
	match mouse_mask: 
		MOUSE_BUTTON_LEFT:
	...
	for each_child in get_children():
		if each_child is PathSweeperPressTypeControlSelector:
			if each_child == _active_primary:
				each_child.apply_mouse_mask(MOUSE_BUTTON_LEFT)
			else: 
				each_child.remove_mouse_mask(MOUSE_BUTTON_LEFT)
				
			if each_child == _active_secondary:
				each_child.apply_mouse_mask(MOUSE_BUTTON_RIGHT)
			else: 
				each_child.remove_mouse_mask(MOUSE_BUTTON_RIGHT)
				
			if each_child == _active_tertiary:
				each_child.apply_mouse_mask(MOUSE_BUTTON_MIDDLE)
			else: 
				each_child.remove_mouse_mask(MOUSE_BUTTON_MIDDLE)
