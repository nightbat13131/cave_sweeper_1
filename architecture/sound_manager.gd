...
@export var highlight_cell_change_sfx : AudioStream
...
@export var found_exit_sfx : AudioStream
...
@export_category("Monster Sounds")
@export var monster_attack_sfx : AudioStream
@export var monster_die_sfx : AudioStream
...
@onready var sfx_players_general: Node = %SFXPlayers
@onready var sfx_players_cell: Node = %SFXPlayers_Cell
...
var _sfx_player_general_index := 0: 
	set(value): _sfx_player_general_index = value % sfx_players_general.get_child_count()

var _sfx_player_cell_index := 0: 
	set(value): _sfx_player_cell_index = value % sfx_players_cell.get_child_count()
...
static func request_sfx_via_enum(enum_: Utilties.SFX, pitch := 1.0, delay := 0.0) -> void:
	if _current_manager:
		if delay > 0.0:
			await _current_manager.get_tree().create_timer(delay).timeout
...
func _request_sfx_via_enum(enum_: Utilties.SFX, pitch : float) -> void:
	...
	match enum_:
		...
		Utilties.SFX.SETTING_OPEN:
			sound = screen_opening
		Utilties.SFX.SETTING_CLOSE:
			sound = screen_closing
		...
		Utilties.SFX.HIGHLIGHTED_CELL_CHANGED:
			sound = highlight_cell_change_sfx
		...
		Utilties.SFX.DANGER_MONSTER_ATTACK:
			sound = monster_attack_sfx
		Utilties.SFX.DANFER_MOSTER_DIED:
			sound = monster_die_sfx
		
		_:
			prints("no sound enum found", enum_)
			return
	_request_sfx(sound, pitch, enum_)
...
func _request_sfx(sound: AudioStream, pitch: float, enum_:= Utilties.SFX.NA) -> void:
	pitch += randf_range(-.15,.15)
	var target_player : AudioStreamPlayer
	if enum_ == Utilties.SFX.HIGHLIGHTED_CELL_CHANGED:
		target_player = sfx_players_cell.get_child(_sfx_player_cell_index)
		_sfx_player_cell_index += 1
	else:
		target_player = sfx_players_general.get_child(_sfx_player_general_index)
		_sfx_player_general_index += 1
	target_player.stop()
